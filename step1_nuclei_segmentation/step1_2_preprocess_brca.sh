...
#$ -wd /home/ucbtsp5/Scratch/24Exp01_CellViT_seg/OUTPUT
...
# 检查Python脚本是否成功执行
if [ $? -eq 0 ]; then
  # 更新CSV文件，将processing_completed列设置为True
  awk -F',' -v row="$SGE_TASK_ID" 'BEGIN{OFS=","} NR == row {$5="True"} 1' "$CSV_FILE" > temp && mv temp "$CSV_FILE"
else
  echo "Error: Python script failed for SGE_TASK_ID $SGE_TASK_ID"
  exit 1
fi